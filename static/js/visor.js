...
(() => {
  ...
  // static/js/canvas.ts
  var RULER_PX, renderOutput, cargarImagenGenerada, updateZoomLabel, updateGridToggleVisibility, setScale, updateSlider, getPixelHex;
  ...
  var setOrigFile = (file, dropZone) => {
    ...
    tempImg.onload = () => {
      if (tempImg.width > MAX_IMAGE_DIM || tempImg.height > MAX_IMAGE_DIM) {
        alert(
          `La imagen no puede superar ${MAX_IMAGE_DIM}\xD7${MAX_IMAGE_DIM} p\xEDxeles.
Dimensiones: ${tempImg.width}\xD7${tempImg.height}.`
        );
        state.origFile = null;
        const btn2 = document.getElementById("btn-generar");
      ...
      if (uniqueColors >= 9e3) {
        alert(
          `La imagen no puede tener 9000 colores o m\xE1s.
